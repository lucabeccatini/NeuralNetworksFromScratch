...
path_plots = "/home/lb_linux/NeuralNetworksFromScratch"



##################################################
# test back-propagation NNFS vs keras
##################################################
...
if (test_2==True):
    ...
    for seed_all in seeds:
        ...
        history = model_k.fit(X_train, Y_train, validation_data=(X_val, Y_val), epochs=100, batch_size=100, callbacks=[callback]) 

        save_layers = "{}".format(layers[0])
        for i in range(1, len(layers)):
            save_layers += "x{}".format(layers[i])

        with PdfPages("{}/PlotMoons_layers{}_seed{}.pdf".format(path_plots, save_layers, seed_all)) as pdf: 

            # plot training and validation
            fig, axs = plt.subplots(2, figsize=(8.27, 11.69))             
            axs[0].plot(model_s.loss_train, color='blue', label="training")
            axs[0].plot(model_s.loss_val, color='red', label="validation")
            axs[0].set_title('model loss')
            axs[0].set(xlabel="epoch", ylabel="loss")
            axs[0].legend(['Train', 'Validation'], loc='best')
            axs[1].plot(history.history['loss'], color='blue', label="training")
            axs[1].plot(history.history['val_loss'], color='red', label="validation")
